...
def parens_update(current_output, next_input):
    ...
    """
    # print(type(current_output), current_output)
    # print(type(next_input), next_input)
    if current_output == 0:
        return next_input
...
def parens_match_scan(mylist):
    ...
    for element in mylist:
        mapped.append(paren_map(element))
    if mapped[0] == -1 or mapped[len(mapped) - 1] == 1:
        return False
    if scan(plus, 0, mapped)[1] != 0 and reduce(plus, 0, mapped) != 0:
        return False
    else:
        return True
...
def scan(f, id_, a):
    ...
    """
    return (
        ([reduce(f, id_, a[:i + 1]) for i in range(len(a))], reduce(f, id_, a))
    )
...
def parens_match_dc_helper(mylist):
    ...
    if len(mylist) == 1:
        if mylist[0] == '(':
            ...
            return 0, 0
    n = len(mylist)
    left = parens_match_dc_helper(mylist[n // 2:])
    right = parens_match_dc_helper(mylist[: n // 2])
